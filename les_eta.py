...
def _latest_run_dir(repo: Path) -> Path:
    ...
    cands = [Path(p) for p in glob.glob(patt)]
    if not cands:
        raise FileNotFoundError(f"No run dirs match {patt}")
    return max(cands, key=lambda p: p.stat().st_mtime)

def _pick_log(run_dir: Path) -> Path:
    a = run_dir / "log.spinup"
    b = run_dir / "log.capture"
...
def estimate_eta(repo: str | Path = ".", window: int = 200) -> ETAResult:
    ...
    if len(pairs) < 5:
        raise RuntimeError("Not enough data yet")
...
def main() -> None:
    ...
    ap.add_argument("--window", type=int, default=200, help="number of recent ClockTime samples")
    ap.add_argument("--follow", action="store_true", help="repeat periodically")
    ap.add_argument("--interval", type=float, default=15.0, help="seconds between updates in --follow")
    args = ap.parse_args()

    while True:
        try:
            res = estimate_eta(args.repo, args.window)
            _print(res)
