...
def main():
    ...
    ap.add_argument("--z-start", type=float, default=None,
                    help="first target z in meters. default: z_step/2")
    ...
    if not frames:
        raise RuntimeError("No frames selected. Check --t0/--t1/--stride.")
    ...
    print(f"[info] z_step={args.z_step}m z_selected(m)={np.round(zc,3).tolist()}")
